...
namespace Fitnes.Application.EntityTypeConfiguration
{
    public class ConsumerTypeConfiguration : IEntityTypeConfiguration<Consumer>
    {
        public void Configure(EntityTypeBuilder<Consumer> builder)
        {
            ...
            builder.HasMany(x => x.Services).WithOne().HasForeignKey(x => x.ConsumerId);
            builder.HasMany(x => x.Messages).WithOne().HasForeignKey(x => x.ConsumerId);
            builder.HasMany(x => x.Cards).WithOne().HasForeignKey(x => x.ConsumerId);
            builder.HasOne(x => x.Teacher).WithMany(x => x.Consumers).HasForeignKey(x => x.TeacherId);
            builder.HasOne(x => x.User).WithOne().HasForeignKey<Consumer>(x => x.UserId);
        }
    }
}
